...
8. [x] P2 - Action buttons and nav links below 44px touch target minimum
   - Agent: watson-qa-verbindige
...
1. [x] P1 - `transform: none !important` in reduced-motion media query breaks Toast and EmojiPool tooltip centering
...
1. [x] P2 - Extract `shuffleArray` utility function
   ...
   - Triage note (2026-04-21): **No duplication remains.** Confirmed `shuffleArray` exists only in `useVerbindige.ts:69`. No other consumers. Can be marked `[x]` by roadmap-worker as the original issue (cross-game duplication) no longer applies.
